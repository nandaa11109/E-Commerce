    ...
    return (
        <div className="bg-white min-h-screen max-w-6xl mx-auto p-4 md:p-7">
            ...
            <div className="flex flex-wrap justify-between items-center mb-6">
                <h1 className="text-2xl md:text-3xl text-black font-bold">CheckOut</h1>
                <button 
                    className="border border-black px-4 py-2 rounded-xl hover:bg-gray-200 transition cursor-pointer text-sm md:text-base"
                    onClick={() => navigate("/")}
                >
                    HomePage
                </button>
            ...
            <div className="overflow-x-auto">
                ...
                <div className="grid grid-cols-12 gap-4 border-t pt-4 font-semibold text-gray-700 text-center mt-6">
                    <div className="col-span-9 text-right">Total</div>
                    <div className="col-span-3 text-center">Rp {cartItems.reduce((total, item) => total + Number(item.price.replace(/\./g, "")) * item.quantity, 0).toLocaleString("id-ID")}</div>
                </div>

                <div className="mt-10 flex justify-between items-center">
                    <button onClick={() => navigate("/CartPage")} className="text-[#003092] border border-[#003092] px-4 py-2 rounded-lg cursor-pointer">Back</button>
                    <button className="text-white bg-[#003092] px-4 py-2 rounded-lg cursor-pointer hover:bg-gray-200 hover:text-[#003092]">Confirm Purchase</button>
                </div>
        </div>
    );
};
